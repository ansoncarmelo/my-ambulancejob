...
function OpenAmbulanceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'ambulance_actions', {
		...
		elseif data.current.value == 'boss_actions' then
			TriggerEvent('esx_society:openBosFreeHongKongsMenu', 'ambulance', function(data, menu)
				menu.close()
			end, {wash = false})
		end
		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				elements = {
					...
					{label = "開單", value = 'bill'},
					{label = "搜尋附近患者", value = 'search'},
					--{label = "拿出物品(需要站在緊急車輛附近)", value = 'scenemenu'}
			}}, function(data, menu)
				...
				elseif closestPlayer == -1 or closestDistance > 1.0 then
					...
					elseif data.current.value == 'scenemenu' then
						ExecuteCommand("prop")
...
function OpenPharmacyMenu()
	ESX.UI.Menu.CloseAll()
	TriggerServerEvent('mf-inventory:createambulanceStorage', 'ambulance')
	exports["mf-inventory"]:openOtherInventory('ambulance')
